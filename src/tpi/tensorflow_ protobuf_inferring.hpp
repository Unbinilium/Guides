...
class model {
 public:
  inline void load(const char* model_path) {
    ...
    buf->data = data;
    buf->length = f_size;
    buf->data_deallocator = [](void* data, size_t length) -> void {
      std::free(data);
    };
  ...
  inline void session(const char* input_oper_name, const int input_index,
                      const char* output_oper_name, const int output_index) {
    input = reinterpret_cast<tf::TF_Input*>(std::malloc(sizeof(tf::TF_Input)));
    input[0] = {tf::TF_GraphOperationByName(graph, input_oper_name),
                input_index};
    ...
    output =
        reinterpret_cast<tf::TF_Output*>(std::malloc(sizeof(tf::TF_Output)));
    output[0] = {tf::TF_GraphOperationByName(graph, output_oper_name),
                 output_index};
